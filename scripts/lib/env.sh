...
7. Installer behavior: DEPLOY_MODE, SKIP_REPO_SYNC
8. Worker VM provisioning: bridge, worker IPs, VM sizing
9. Observability: Grafana/Prometheus install, Grafana host, and Grafana admin login
...
EOF_MENU
    ...
    _env_prompt GRAFANA_ADMIN_USER "Grafana admin username" 1 0
    _env_prompt GRAFANA_ADMIN_PASSWORD "Grafana admin password (blank lets Helm keep/generate it)" 0 1
    _env_prompt GRAFANA_ADMIN_SECRET_NAME "Grafana admin secret name" 1 0
    ;;
    10)
    normalize_loaded_env
    _write_env_file "$ENV_FILE"
...
if [ -n "${GRAFANA_ADMIN_PASSWORD:-}" ]; then
  [ -n "${GRAFANA_ADMIN_USER:-}" ] || fatal "GRAFANA_ADMIN_USER is required in $ENV_FILE when GRAFANA_ADMIN_PASSWORD is set"
  [ -n "${GRAFANA_ADMIN_SECRET_NAME:-}" ] || fatal "GRAFANA_ADMIN_SECRET_NAME is required in $ENV_FILE when GRAFANA_ADMIN_PASSWORD is set"
fi
...
export TELEGRAM_BOT_TOKEN TELEGRAM_CHAT_ID RUNTIME_DATA_DIR SKIP_HELP_DOCS_BUILD GIT_CLEAN SKIP_REPO_SYNC NONINTERACTIVE DEPLOY_MODE DOCKER_BIN REDIS_ENABLED REDIS_URL REDIS_REQUIRED REDIS_STORAGE_CLASS REDIS_SIZE REDIS_SPREAD_RECREATE_PVCS
...
export OBSERVABILITY_NAMESPACE OBSERVABILITY_INSTALL_STACK OBSERVABILITY_STACK_CHART_VERSION GRAFANA_HOST GRAFANA_ADMIN_USER GRAFANA_ADMIN_PASSWORD GRAFANA_ADMIN_SECRET_NAME
