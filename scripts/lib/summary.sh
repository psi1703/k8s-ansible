...
summary_grafana_url() {
  local host="${GRAFANA_HOST:-grafana-srvotptest26.init-db.lan}"
  local scheme="http"

  if [ "${GRAFANA_TLS_ENABLED:-${TLS_ENABLED:-0}}" = "1" ]; then
    scheme="https"
  fi
  ...
  if [ -n "${GRAFANA_URL:-}" ]; then
    summary_normalize_url "$GRAFANA_URL"
    return 0
  fi
...
write_install_report() {
  ...
  local grafana_url_summary="${GRAFANA_URL_SUMMARY:-}"

  if [ -z "$report_file" ]; then
    report_file="$(summary_install_report_path)"
  fi
  if [ -z "$grafana_url_summary" ]; then
    grafana_url_summary="$(summary_grafana_url)"
  fi
...
Layered configuration summary
-----------------------------
Application layer
  Portal URL:              $(summary_normalize_url "${PORTAL_URL:-}")
  Guide pop-out:           /guide.html?step=<step>&page=<page>
  Namespace:               $namespace
...
Observability layer
  ...
  Install stack:           ${OBSERVABILITY_INSTALL_STACK:-1}
  Grafana URL:             $grafana_url_summary
  Grafana host:            ${GRAFANA_HOST:-grafana-srvotptest26.init-db.lan}
  Stack chart version:     ${OBSERVABILITY_STACK_CHART_VERSION:-85.0.1}
...
Monitor configuration
  ...
  PHONE_PING_INTERVAL:     ${PHONE_PING_INTERVAL:-not-set}s
  PHONE_OFFLINE_THRESHOLD: ${PHONE_OFFLINE_THRESHOLD:-not-set}s
EOF_REPORT
  ...
  if summary_kubectl_available; then
    ...
    summary_append_command_output "$report_file" "OTP Relay Redis Sentinel pods" summary_kubectl get pods -n "$namespace" -l app=otp-redis-sentinel -o wide
    summary_append_command_output "$report_file" "OTP Relay Redis HAProxy pods" summary_kubectl get pods -n "$namespace" -l app=otp-redis-haproxy -o wide
    ...
    if [ "${OBSERVABILITY_INSTALL_STACK:-1}" = "1" ]; then
      summary_append_command_output "$report_file" "Observability resources" summary_kubectl get pods,svc,ingress,servicemonitor -n "$observability_namespace" -o wide
    fi
  else
    {
      printf '\nCluster status\n--------------\n'
...
Observability layer
  ...
  Install stack:     ${OBSERVABILITY_INSTALL_STACK:-1}
  Stack chart:       ${OBSERVABILITY_STACK_CHART_VERSION:-85.0.1}
...
Useful commands:
  ...
  k3s kubectl get pods,svc,ingress,servicemonitor -n $observability_namespace -o wide
