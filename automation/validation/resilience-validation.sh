...
# Layer: live resilience validation.
#
# Purpose:
# - Validate that the running OTP Relay Kubernetes deployment is healthy.
# - Optionally validate controlled restarts, Redis pod recovery, and worker drain recovery.
# - Keep destructive tests opt-in so normal validation is safe by default.
#
# Default behavior:
# - Non-destructive health checks only.
# - Does not restart deployments, delete Redis pods, drain nodes, or mutate the cluster.
#
# Destructive behavior:
# - Enabled only with RUN_DESTRUCTIVE_TESTS=1.
# - Requires interactive confirmation unless ASSUME_YES=1 is also set.
#
# Examples:
...
#   REQUIRE_REAL_OTP_CONFIRMATION=1 bash automation/validation/resilience-validation.sh
...
confirm_destructive_tests() {
  ...
  if [ "$RUN_DESTRUCTIVE_TESTS" != "1" ]; then
    info "RUN_DESTRUCTIVE_TESTS is not 1; restart/delete/drain tests will be skipped."
    return 0
  fi
...
curl_portal_once() {
  ...
  "$CURL_BIN" "${curl_args[@]}" "$url" >"$body_file" 2>"$error_file"
}

check_portal_ready() {
  local url host_header attempt rc body error body_file error_file
  ...
  for attempt in $(seq 1 "$PORTAL_CHECK_ATTEMPTS"); do
    if curl_portal_once "$url" "$host_header" "$body_file" "$error_file"; then
...
check_redis_stack() {
  ...
  elif kubectl_cmd -n "$NAMESPACE" exec "$redis_pod" -- redis-cli -p "$REDIS_PORT" PING | grep -q '^PONG$'; then
  ...
  elif kubectl_cmd -n "$NAMESPACE" exec "$sentinel_pod" -- redis-cli -p "$SENTINEL_PORT" SENTINEL get-master-addr-by-name "$SENTINEL_MASTER_NAME" | grep -qE '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$'; then
...
check_storage_and_pdbs() {
  local pdbs bad_pdbs no_requests
...
check_observability() {
  local loki_pods grafana_pods prometheus_pods
  ...
  run_cmd kubectl_cmd -n "$OBSERVABILITY_NAMESPACE" get ingress -o wide || true
  ...
  info "Expected Helm releases include: $KPS_RELEASE, $LOKI_RELEASE, $ALLOY_RELEASE"
}
...
check_repo_syntax() {
  ...
  for file in \
    ...
    "$repo_root/scripts/lib/observability.sh" \
...
files = [
    ...
    root / "k8s/observability/grafana-ingress.yaml",
]
for file in files:
    if file.exists():
...
wait_for_recovery() {
  ...
  local elapsed=0 before_count last_index
...
detect_worker_nodes() {
  ...
  kubectl_cmd get nodes --no-headers 2>/dev/null | awk '$3 !~ /control-plane|master/ {print $1}'
}
